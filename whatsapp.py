...
# --------------------------
WHATSAPP_API_VERSION = "v1.0"  
PHONE_NUMBER_ID = "628243637037288"  
WHATSAPP_TOKEN = "699072222543098"  
VERIFY_TOKEN = "EAAIBfGzHL4EBOyQnZAd4JbAqiRyY0pyqugAZBP5OP4zyw5YVhHk6GvGzuKIUtyLsgoZCOXEcgPAKQwJZB0qry2W4JQPbFCC4bAHKBlMqm489NZAs6P9uRZBNeilXgIZCSNaVua0oaElaQBlZBndPW8ieOoyt5VNWTm1IJFHa7R8zfL3wslAyDGOp2Ui072nE8jJOqPuA4DDZByEeVngsfj1BPiK9tdjRrQTZAAIGM46ZBB5GwPC1KWlEVgZD"  # Custom verification token for webhook setup
META_APP_SECRET = "YOUR_META_APP_SECRET"  # For validating webhook signatures
...
def get_dish_by_name(query, lang="en"):
    ...
    if lang == "ar":
        ...
        results = arabic_bm25.get_top_n(norm_query, menu, n=1, include_scores=True)
        ...
        # If no exact match, use BM25
        results = english_bm25.get_top_n(norm_query, menu, n=1, include_scores=True)
        if results and len(results) > 0:
            # Check if results is non-empty and has the expected (dish, score) tuple format
            if isinstance(results[0], tuple) and len(results[0]) > 1:
                if results[0][1] > 0.5:  # Score threshold
...
# ---------------------------
# Menu Display Function for WhatsApp
# ---------------------------
def show_menu(lang="en"):
    ...
    if lang == "ar":
        intro_text = "🍽️ هذه هي قائمتنا! \n\nهل ترغب في تقديم طلب؟ )"
    else:
        intro_text = "🍽️ Here's our menu! \n\nWould you like to place an order? "
...
# ---------------------------
# Response Enhancement with Groq
# ---------------------------
def enhance_response_with_groq(response_message, user_input, lang="en"):
    ...
    GROQ_API_KEY = os.environ.get("GROQ_API_KEY", "put your api here")
...
def send_whatsapp_menu(to, menu_response):
    """Send menu images and text via WhatsApp API"""
    # First send the intro text
    send_whatsapp_message(to, menu_response["text"])
    
    # Then send each menu image
    for image_url in menu_response["images"]:
        send_whatsapp_image(to, image_url)

def send_whatsapp_location_request(to, lang="en"):
    """Send a request for location via WhatsApp API"""
    url = f"https://graph.facebook.com/{WHATSAPP_API_VERSION}/{PHONE_NUMBER_ID}/messages"
    ...
    # Define the message text based on language
    if lang == "ar":
        message_text = "📍 الرجاء مشاركة موقعك باستخدام ميزة مشاركة الموقع في واتساب"
    else:
        message_text = "📍 Please share your location using WhatsApp's location sharing feature"
    ...
    try:
        ...
        logger.error(f"Error sending WhatsApp location request: {e}")
        return None

# ---------------------------
# Main Message Processor (for WhatsApp)
# ---------------------------
def process_message(user_number, user_input, latitude=None, longitude=None):
    ...
    if user_states[user_number] == "awaiting_menu_or_order":
        if "menu" in user_input or "قائمة" in user_input:
            # Menu display needs special handling for WhatsApp
    ...
    # --- 2. Awaiting Dish Selection ---
        ...
        if user_input in ["done", "finish", "انتهيت", "تم"]:
            ...
            if dish:
                ...
                if lang == "ar":
                    suggestions = arabic_bm25.search(normalize_arabic(dish_name_query), menu, n=3, include_scores=True)
                    suggestions = [item for item, score in suggestions if score > 0.2]
                else:
                    suggestions = english_bm25.search(dish_name_query, menu, n=3, include_scores=True)
                    suggestions = [item for item, score in suggestions if score > 0.2]
    ...
    # --- 8. Awaiting Payment Method (For Delivery Only) ---
        ...
        if "cash" in user_input or "نقد" in user_input:
            ...
            # Special WhatsApp location request
            send_whatsapp_location_request(user_number, lang)
    ...
    # --- 9. Awaiting Delivery Location (For Delivery Only) ---
        ...
        if latitude and longitude:
            ...
            response = ""
            if lang == "ar":
                response = "❓ الرجاء مشاركة موقع التوصيل باستخدام ميزة مشاركة الموقع في واتساب أو كتابة عنوانك."
            else:
                response = "❓ Please share your delivery location using WhatsApp's location sharing feature or type your address."
            
            # Re-send location request
            send_whatsapp_location_request(user_number, lang)
    ...
    # --- 10. Awaiting Name (Finalize Order) ---
        ...
        reset_user(user_number)  # Reset for next order
...
def verify_webhook_signature(request_data, signature_header):
    """
    Verify that the webhook request came from Meta by checking the signature
    """
    ...
    try:
        ...
        # Compute a SHA-256 HMAC signature using your app secret
...
def handle_whatsapp_message(data):
    ...
    """
    try:
        # Extract the message from webhook data
        entry = data.get('entry', [])
        if not entry or not entry[0].get('changes', []):
            return None
        
        changes = entry[0]['changes'][0]
        if not changes.get('value', {}).get('messages', []):
            return None
        ...
        # User location - extract coordinates if available
        latitude = None
        longitude = None
        user_input = ""
        ...
        if message_type == 'location':
            ...
            location = message_info.get('location', {})
            latitude = location.get('latitude')
            longitude = location.get('longitude')
            user_input = ""  # No text in a location message
        elif message_type == 'text':
            # Extract text message
            user_input = message_info.get('text', {}).get('body', "")
        else:
            # For other message types (image, video, etc.), set a placeholder
            user_input = f"[Received {message_type}]"
        
        # Process the message
        response_content = process_message(from_number, user_input, latitude, longitude)
        
        # Send the response back to the user
        if isinstance(response_content, dict) and "images" in response_content:
            # Handle menu with images
            send_whatsapp_menu(from_number, response_content)
        else:
            # Handle regular text message
            send_whatsapp_message(from_number, response_content)
        
        # Log the interaction
        log_message(from_number, user_input, response_content)
...
# ---------------------------
# Flask Routes for WhatsApp Integration
# ---------------------------
...
def receive_webhook():
    ...
    # Verify that this request is from Meta
    ...
    # Check if this is a WhatsApp message or another type of webhook
    if data.get('object') == 'whatsapp_business_account':
        # Process the message
        result = handle_whatsapp_message(data)
        if result:
            return jsonify(result), 200
    
    # Return a 200 OK for all webhooks to acknowledge receipt
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not menu:
            logger.warning("Menu could not be loaded. Using empty menu.")
    except Exception as e:
        logger.error(f"Error during initialization: {e}")
    
    # Start the Flask application
    logger.info(f"Starting WhatsApp restaurant bot on port {port}")
